...
Lib.fetchAPI = function *fetch(path) {

    let json = null;
    try {
        let response = yield request(baseUrl+ apiVersion + path);
        json = JSON.parse(response.body);
    }
    catch(e) {
        Lib.logError('Lib.fetch', e);
    }

    return json;
};

Lib.fetchS3 = function *fetchCoverFallback(name) {

    let json = null;
    try {
        let response = yield request('https://s3.amazonaws.com/newsblock/json/'+ name +'_latest.json');
        json = JSON.parse(response.body);
    }
    catch(e) {
        Lib.logError('Lib.fetchS3', e);
    }
    return json;
};
